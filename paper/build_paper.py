...
def build():
    ...
    add_note(doc, "Draft status", "Prepared from measurements executed in Codex on 23 August 2026, with supplementary checks added on 25-26 August 2026: construct-validity (Section 4.5), a third language (Section 4.6), calibrated batching (Section 4.7), resource-usage and second-distribution supplements (Section 4.8), and an independent Windows reproduction (Section 4.9). This manuscript is intended for internal academic review and methodological refinement before journal submission.")
    ...
    abstract = (
        ...
        "post-deletion checksums, including five supplementary checks added across 25-26 August 2026 that test the primary "
    ...
    bullet(doc, "Peak-memory, coarse hardware-counter and second-input-distribution supplements that close four further items from the original future-work list within a single Mac's reach.")
    bullet(doc, "An independent reproduction on a second, separately owned Windows machine -- closing the fifth of the six original future-work items -- testing whether the primary Python/C++ comparison is an Apple Silicon artifact rather than leaving single-machine scope as an unaddressed limitation.")
    ...
    add_para(doc, "This reproducible pilot showed that theoretical growth and observed runtime can be taught together. Sequential search and deletion batches grew close to quadratically when both collection size and operation count increased, while hash batches grew close to linearly. Python and C++ showed substantial but operation-dependent runtime differences, and all correctness outputs agreed. A supplementary Java run showed that a JIT-compiled, managed-runtime language does not sit uniformly between the interpreted and compiled extremes, slower than Python on four of the twelve structure-operation combinations despite being faster overall; the run also exposed measurement dispersion far higher than the primary design's, a methodological finding in its own right. An independent Windows reproduction (Section 4.9) then confirmed that C++ was faster than Python in all twelve structure-operation combinations on a different machine too, though the magnitude of the gap varied considerably by hardware and toolchain rather than transferring unchanged. The study also exposed a measurement weakness: several optimized C++ workloads were too short for reliable single-shot timing.", align=WD_ALIGN_PARAGRAPH.JUSTIFY)
    ...
    add_para(doc, "No human-participant, personal or sensitive data were used. Generative AI (Codex and Claude) assisted with repository scaffolding, code review, execution orchestration, analysis scripting and preparation of this draft, including the supplementary construct-validity and third-language checks added on 25 August 2026 and the calibration, resource-usage, second-distribution and independent-reproduction supplements added on 26 August 2026. The independent Windows reproduction (Section 4.9) was executed by the second author on his own machine, not by the AI assistant. All numerical claims in the manuscript were generated programmatically from the preserved raw CSV; missing measurements were not fabricated. The named authors have independently reviewed the code, results, citations and wording, and approved authorship contributions; this disclosure should still be adapted to the selected journal's specific policy at the time of submission.", align=WD_ALIGN_PARAGRAPH.JUSTIFY)
